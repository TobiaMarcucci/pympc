import numpy as np
from mpc_tools.optimization import linear_program
import time

print 'Case study for the (home-made 20-lines-of-code) PNNLS solver for linear programs.\n'
...
tic = time.clock()
x_inside = linear_program(f, A, b, solver='pnnls')[0]
toc = time.clock()
...
tic = time.clock()
x_star, f_star = linear_program(f, A, b, solver='pnnls')
toc = time.clock()
